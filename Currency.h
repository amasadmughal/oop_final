...
{
private:
	double rate_wrt_USD;				//currency's rates w.r.t USD
	string Currency_name;			//currency's name
	string OBV;						//currency's obrivations
	double ConvertedAmount;			//the resultant amount that will we give user after conversion
	...
	double CurrencyRates[21];		//string array to store currency rates that would be fetched from file
public:
	//constructors
	Currency();
	Currency(string, string, double);

	void setConvertedAmount(string&, string&, double&);
...
void Currency::setConvertedAmount(string& currency1, string& currency2, double& amount)
{
